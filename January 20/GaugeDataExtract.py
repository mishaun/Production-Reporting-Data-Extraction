...
gauge_sheet_directory = abspath + '/Gauge Sheets - January 2020'
...
data_map_path = 'Jan 2020 - MPLP - PR EDI.xlsm'
data_map = pd.read_excel(data_map_path, header = 4, sheet_name = "Working Spreadsheet Jan 20", usecols = 'C:AQ')
    ...
    try:
        ...
        for item in regex:
            for name in filenames:
                #Only looking at first at x% of letters of regex
                item_length = round(len(item)*0.9)
    ...
    if type(wb) != type(None):
        ...
        if well["Monthly Tabs"]=="Y" and well["Multiple Years"] == "Y":
            ...
            for name in sheetnames:
                #for 2020, having to check the sheet name's last 2 digits to avoid pulling January 2019, etc, due to '20' being in the name
                if reportMonth.upper() in name.upper() and str(reportYear)[-2:] in name[-2:]:
                    getsheet = name
                    break
